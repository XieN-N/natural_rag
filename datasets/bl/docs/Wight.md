# Wight

[Aside block] Wight
-------------------

### Health

76 ( x 38)

### Damage

Contact Easy: 4 
Contact Normal: 6 
Contact Hard: 9 
Possession: 3 
### Drops

Wight's Heart  
*(See Drops for more info)*

### Experience

10

### ID

wight

A Wight in-game.

*"One by one, the people of the Betweenlands were transformed into Wights; beings of pure hatred, neither living nor dead."*

> *-From The Story of The Betweenlands*

**Wights** are white, ghostly hostile mobs that spawn occasionally throughout The Betweenlands.

Contents
--------

* 1 Spawning
  + 1.1 Spawners
  + 1.2 Summoning
* 2 Behavior
  + 2.1 Possession
* 3 Strategy
* 4 Drops
* 5 Lore
* 6 Trivia
* 7 Advancements
* 8 Sounds
* 9 Video
* 10 History

Spawning
----------

Wights spawn uncommonly in any biome on the surface of The Betweenlands, *except* for Deep Waters and Swamplands Clearing biomes. They can also spawn anywhere in the Cavern and Lake Cavern layers.

### Spawners
Wights spawn from some Monster Spawners in Underground Oases, Shrines, Cragrock Towers, and the Wight Fortress.

### Summoning
Wights are occasionally summoned by the Primordial Malevolence, but with only 30 ( x 15) health.

Behavior
----------

Wights wander around slowly and cause visual distortion of the area around them. When they notice the player they will look up, mouths agape, and attack on sight with melee. They have a detection range of 80 blocks.

Wights have a chance to spawn applied with a random Amulet, which will give it Middle Gem Circle benefits depending on the type applied. This amulet will not drop upon their death.

If a Wight contacts Tar, it will transform into a Tar Beast after a few seconds. This can be cancelled by knocking the wight out of the tar.

### Possession
Wights will occasionally transform into a disembodied head that is able to fly, pass through blocks, and "possess" players or Swamp Hags by grabbing onto their head, circling it as it does so. Wights are more likely to transform at lower health and when other Wights are nearby, but will not transform to chase the player unless hit by them. Once the Wight possesses an entity, it will begin to shoot homing particles outward, dealing magic damage that bypasses armor to the player as well as other nearby entities (excluding hostile Betweenlands entities.) The Wight's hitbox completely surrounds the player's head, meaning most of the player's attacks can only hit the Wight. The Wight is still vulnerable to damage and can be removed after enough attacks, if it is not killed in doing so. The wight will also eventually leave possession mode after enough time passes.

When a Wight possesses a Swamp Hag, it will not damage its host, and will continuously emit projectiles at the player until either it or the Swamp Hag are killed. It can be attacked separately from the mob it is possessing.

Strategy
----------

The player can avoid being targeted by a Wight by sneaking when around it, but cannot damage it in this state without attracting its attention. It will not stop targeting the player if it spotted them before sneaking, unless they sneak after breaking direct line of sight with it. Additionally, if a sneaking player is afflicted with Stench, Wights will still target them on sight.

Wights can be killed in one hit using a rare loot weapon, the Wight's Bane.

When the player wears a Skull Mask, Wights are unable to possess them, and nearby possessed Swamp Hags will not shoot projectiles.

Wearing Ancient Armor will reduce the magic damage dealt by possession projectiles by 12.5% per piece, for a total of 50% with the full set.

Wights cannot be repelled by a Repeller.

Drops
-------

The drops from Wights are derived from the wight loot table.

The following is a list of all of the items that can be dropped by Wights, including their rolls, quantities, and weights.

| **Wight** Drops | | |
| --- | --- | --- |
| **Dropped Item** | **Quantity** | **Weight** |
| 1 Roll, if killed by player: | | |
| **Nothing** | N/A | 2/3 (66.67%) |
| **Wight's Heart** | 1 (+0-1 per Looting level) | 1/3 (33.33%) |
| 2 Rolls, if killed by player, Spook event only: | | |
| **Nothing** | N/A | 1/4 (25%) |
| **Blue Candy** | 1 | 1/4 (25%) |
| **Red Candy** | 1 | 1/4 (25%) |
| **Yellow Candy** | 1 | 1/4 (25%) |
| 1 Roll, if killed by player, Winter event only: | | |
| **Nothing** | N/A | 1/2 (50%) |
| **Mince Pie** | 1 | 1/2 (50%) |

Visit the Loot Tables page for more information on loot.

Lore
------

Wights were once members of a peaceful civilization that resided in the Betweenlands. However, the dimension was transformed into a nightmare when the dark energies of the Nether surged through a portal that was opened by a Druid. Through this process, the people were transformed into Wights, spirit-like entities driven by an uncontrollable anger. The Druid himself became the Prime Wight and has authority over most of the other Wights.

Trivia
--------

* "Possesion" mode is actually referred to as being "Volatile" in the code.

Advancements
--------------

| Advancement | In-game Description | Parent | Actual Requirements | ID |
| --- | --- | --- | --- | --- |
| Fearless Fighter | Slay a Swamp Hag, a **Wight**, a Tar Beast, a Sludge, and a Peat Mummy | Chopping Boy | Kill a Swamp Hag, **Wight**, Tar Beast, Sludge, and Peat Mummy (does not have to be at the same time) | fighter /fearless\_fighter |
| Whhyyy? | Slay a **Wight** with a Wight's Bane | Chopping Boy | Same as description | fighter /whhyyy |

Sounds
--------

| Sound | Subtitles | Source | Description | ID |
| --- | --- | --- | --- | --- |
| https://the-betweenlands.fandom.com/wiki/File:WightAttack1.ogg https://the-betweenlands.fandom.com/wiki/File:WightAttack2.ogg https://the-betweenlands.fandom.com/wiki/File:WightAttack3.ogg https://the-betweenlands.fandom.com/wiki/File:WightAttack4.ogg https://the-betweenlands.fandom.com/wiki/File:WightAttack5.ogg | Wight wails | Hostile | Plays when a Wight transforms. | wight\_attack |
| https://the-betweenlands.fandom.com/wiki/File:WightDeath.ogg | Wight dies | Hostile | Plays when a Wight dies. | wight\_death |
| https://the-betweenlands.fandom.com/wiki/File:WightHurt1.ogg https://the-betweenlands.fandom.com/wiki/File:WightHurt2.ogg | Wight hurts | Hostile | Plays when a Wight takes damage. | wight\_hurt |
| https://the-betweenlands.fandom.com/wiki/File:WightMoan1.ogg https://the-betweenlands.fandom.com/wiki/File:WightMoan2.ogg https://the-betweenlands.fandom.com/wiki/File:WightMoan3.ogg https://the-betweenlands.fandom.com/wiki/File:WightMoan4.ogg | Wight moans | Hostile | Plays randomly while idle. | wight\_moan |

Video
-------

An animation by Timinations depicting an encounter with a Wight:

Minecraft Animation- Wight Encounter (Betweenlands)

History
---------

* Release 3.8.0: Can now target sneaking players that are afflicted with Stench.
* Release 3.7.1: Increased experience drop from 5 to 10.
* Release 3.7.0: Now transforms into a Tar Beast upon contacting Tar.
* Release 3.3.0: Added Advancements.
* **Beta 1.0.0**: Introduced.