# Valonite Shield

[Aside block] Valonite Shield
-----------------------------

### Type

Equipment

### Damage

5 
### Durability

5000

### Renewable

No

### Stackable

No

### Compostable

No

### ID

valonite\_shield

Valonite Shield when held by the player

The **Valonite Shield** is a special Betweenlands shield.

Contents
--------

* 1 Crafting Recipe
* 2 Blocking
* 3 Repairing
* 4 Middle Gems
* 5 Corrosion
* 6 Advancements
* 7 History

Crafting Recipe
-----------------

| Result | Ingredients | Recipe |
| --- | --- | --- |
| **Valonite Shield** | Valonite Shard (6) + Pitstone |  |

Blocking
----------

Like any shield, the Valonite Shield can be held up to block melee and ranged attacks from the front. The player does not receive any knockback upon blocking. The shield can be temporarily disabled by an axe attack. If the player is inflicted with Slugarm Brew, then attacks will not be blocked.

The Valonite Shield has the unique ability to reflect a small portion of the initial damage an attacker deals back upon them while blocking. In the case of certain projectiles, it can completely reflect them, causing them to be fired back at the same velocity towards their source.

The Valonite Shield also uniquely grants the player the ability to charge into their enemies by holding down shift while blocking with it, then letting go of shift. The player will speed forward, dealing damage to any mobs that they contact from the front and knocking them back greatly. The longer the shield is charged before releasing, the further the player will charge before stopping, up to a maximum of 2 seconds. Note that doing this causes the player to consume hunger at a much faster than usual rate.

Repairing
-----------

Like most other equipment with durability, a damaged Valonite Shield can be repaired to full durability through an Animator. The more that needs to be repaired, the greater the cost of fuel and crystal charge is in the animation process.

| Result | Ingredients | Recipe |
| --- | --- | --- |
| Fully repairs **Valonite Shield** | Damaged **Valonite Shield** + Sulfur (7-16) + Life Crystal (12-36% charge) |  |

Middle Gems
-------------

As with most equipment, the Valonite Shield can be equipped with a Middle Gem as part of the Middle Gem Circle to apply additional buffs to the player using it. The type of gem equipped can be seen in its tooltip. The bonuses provided can be seen on the gem's own tooltip.

| Result | Ingredients | Recipe |
| --- | --- | --- |
| Applies corresponding Middle Gem to **Valonite Shield** | **Valonite Shield** + Any Middle Gem |  |

Corrosion
-----------

The Valonite Shield does not corrode.

Advancements
--------------

| Advancement | In-game Description | Parent | Actual Requirements | ID |
| --- | --- | --- | --- | --- |
| Shield Wall | Equip yourself with a Betweenlands shield | Serious Business | Have a Weedwood, Lurker Skin, Bone, Octine, Syrmorite, **Valonite**, or Dentrothyst Shield in your inventory | fighter /shield\_wall |

History
---------

* Release 3.4.0: Can now be equipped with a Middle Gem.
* Release 3.3.0: Added Advancement.
* Alpha 3.2.0: Can now be fully repaired in an Animator.
* Alpha 3.0.0: Now has the ability to charge the player into enemies and reflect damage and projectiles.
* Beta 2.0.0: Introduced.