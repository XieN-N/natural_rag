...
[Aside block] Dentrothyst Shields
---------------------------------
...
Like any shield, Dentrothyst Shields can be held up to block melee and ranged attacks from the front. The player receives a small amount of knockback upon blocking. The shields can be temporarily disabled by an axe attack. If the player is inflicted with Slugarm Brew, then attacks will not be blocked.

Dentrothyst Shields have the unique ability to prevent the player from receiving effects from splash potions while blocking, including those from thrown Infusions. If the player is inflicted with Slugarm Brew, splash effects will not be blocked either.
...
| Result | Ingredients | Recipe |
| --- | --- | --- |
| Applies corresponding Middle Gem to corresponding **Dentrothyst Shield** | Any **Dentrothyst Shield** + Any Middle Gem |  |
...
* Release 3.3.0:
  + Can now block splash potion effects.
  + Added Advancement.
