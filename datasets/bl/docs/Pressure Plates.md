# Pressure Plates

[Aside block] Pressure Plates
-----------------------------

### Transparency

Yes

### Luminance

No

### Hardness

?

### Blast Resistance

Weedwood: 15.0  
All Others: 30.0

### Tool

Weedwood:  
Hand or Axe  
All Others:  
Any Pickaxe

### Renewable

Yes

### Stackable

Yes (64)

### Flammable

No

### Compostable

No

### ID

See Types

Pressure Plates ingame

**Pressure Plates** in the Betweenlands act functionally the same as vanilla pressure plates.

Contents
--------

* 1 Types
* 2 Crafting Recipes
* 3 Functionality
* 4 Fuel
* 5 History

Types
-------

This is the list of Pressure Plates available, their IDs, and their trigger functionalities:

| Pressure Plates | | | |
| --- | --- | --- | --- |
| **Pressure Plate** | **ID** | **Functionality** |
| Betweenstone Pressure Plate | betweenstone \_pressure\_plate | Can be triggered by players or mobs. |
| Syrmorite Pressure Plate | syrmorite \_pressure\_plate | Can only be triggered by players. |
| Weedwood Plank Pressure Plate | weedwood\_plank \_pressure\_plate | Can be triggered by players, mobs, items or arrows. |

Crafting Recipes
------------------

| Result | Ingredients | Recipe |
| --- | --- | --- |
| Weedwood Plank Pressure Plate | Weedwood Planks (2) |  |
| Betweenstone Pressure Plate | Smooth Betweenstone (2) |  |
| Syrmorite Pressure Plate | Syrmorite Ingot (2) |  |

Functionality
---------------

Like vanilla Pressure Plates, Betweenlands Pressure Plates are capable of detecting specific entities that are on top of them, outputting redstone current when doing so.

Fuel
------

Weedwood Plank Pressure Plates can be used as fuel for the Sulfur Furnace or Dual Sulfur Furnace, smelting 1.5 items per pressure plate block.

History
---------

* Beta 2.0.0: Syrmorite Pressure Plate recipe changed.
* **Beta 1.0.0**: Introduced.